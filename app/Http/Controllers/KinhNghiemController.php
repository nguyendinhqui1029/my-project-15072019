...
class KinhNghiemController extends Controller
{
 ...
 // all component header
    $listContentMaster =[
     ...
     new ContentMaster(true,"mb-1",["1"=>["modules.sub-modules.content-kinh-nghiem","modules.sub-modules.box-kinh-nghiem"],
     ...
     new ContentMaster(false,"mb-1",["modules.sub-modules.phan-trang"])
        ...
        $listData= DB::select('select * from kinhnghiem where TrangThai = ?', [1]);
        $listDataKinhNghiem=[];
            foreach ($listData as $bds) {
                array_push($listDataKinhNghiem,new DataCard($bds->TieuDe,$bds->ID_KinhNghiem,$bds->NoiDungTomTat,$bds->HinhAnh,$bds->NoiDungChiTiet));
            }

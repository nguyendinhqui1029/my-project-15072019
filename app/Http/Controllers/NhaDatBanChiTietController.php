...
class NhaDatBanChiTietController extends MasterController
{
    ...
    public function getnhadatbanchitiet(Request $request){
        $id = $request->id;
        $data = DB::select('select * from dangtin where ID_DangTin = ?', [$id]);
        $var= \View::make('pages.chitietnhadatban',["listHeaderMaster"=>$this->listHeaderMaster,

...
class KinhNghiemChiTietController extends MasterController
{
    ...
    public function getkinhnghiemchitiet(Request $request){
        $id = $request->id;
        $data = DB::select('select * from kinhnghiem where ID_KinhNghiem = ?', [$id]);
        $var= \View::make('pages.kinhnghiemchitiet',["listHeaderMaster"=>$this->listHeaderMaster,

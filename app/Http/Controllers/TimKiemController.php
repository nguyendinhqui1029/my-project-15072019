...
class TimKiemController extends MasterController
{
    ...
    function __construct()
    {
        ...
        $this->listContentMaster =[
            // new ContentMaster(false,"mb-1",["modules.sub-modules.from-tim-kiem"]),
            new ContentMaster(true,"mb-5",["1"=>["modules.sub-modules.from-tim-kiem"],
        "2"=>[]]),
        ];
    }
    public function getTimKiem (Request $request)
    {
        $timkiem = $request->input('txttimkiem');
        $giatien= $request->input('txtgiatien');
        $admin =DB::select('select * from dangtin where  TrangThai = ? and DiaChi=? and GiaBan=?' , [1,$timkiem,$giatien]);
        $var= \View::make('pages.timkiem',["listHeaderMaster"=>$this->listHeaderMaster,
        "listContentMaster"=>$this->listContentMaster,
        "admin"=>$admin,"timkiem"=>$timkiem
        ]);
        return $var;
    }
}
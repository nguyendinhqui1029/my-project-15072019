...
class MauNhaDepChiTietController extends MasterController
{
    ...
    public function getmaunhadepchitiet(Request $request){
        $id = $request->id;
        $data = DB::select('select * from maunhadep where ID_MauNhaDep = ?', [$id]);
        $var= \View::make('pages.maunhadepchitiet',["listHeaderMaster"=>$this->listHeaderMaster,
